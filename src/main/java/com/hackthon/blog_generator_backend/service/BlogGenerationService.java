...
public class BlogGenerationService {
    ...
    /**
     * 이미지 URL과 함께 블로그 콘텐츠 생성
     */
    public String generateBlogContentWithImages(String userInput, String storeInfo, List<String> imageUrls) {
        log.info("이미지 포함 블로그 콘텐츠 생성 요청: userInput={}, imageUrls={}", userInput, imageUrls);
        ...
        try {
            String prompt = createBlogPromptWithImages(userInput, storeInfo, imageUrls);
            log.debug("생성된 이미지 포함 프롬프트: {}", prompt);
            
            ChatCompletionRequest request = ChatCompletionRequest.builder()
                    .model(modelName)
                    .messages(List.of(new ChatMessage("user", prompt)))
                    .maxTokens(1500) // 이미지 포함으로 더 긴 콘텐츠 예상
                    .temperature(0.7)
                    .build();

            log.info("OpenAI API 이미지 포함 콘텐츠 생성 호출 시작...");
            String response = openAiService.createChatCompletion(request)
                    .getChoices().get(0).getMessage().getContent();

            log.info("OpenAI API 이미지 포함 콘텐츠 생성 성공: 길이={}", response.length());
            log.debug("API 응답 내용: {}", response);
            return response;

        } catch (Exception e) {
            log.error("이미지 포함 블로그 콘텐츠 생성 중 오류 발생: {}", e.getMessage(), e);
            throw new RuntimeException("이미지 포함 블로그 콘텐츠 생성에 실패했습니다: " + e.getMessage(), e);
        }
    }
    ...
    private String createBlogPrompt(String userInput, String storeInfo) {
        return String.format("""
            ...
            [출력 형식]
            반드시 JSON 형태로만 응답해주세요. 다른 텍스트나 설명은 포함하지 마세요.
            정확한 JSON 형식을 유지하여 다음 필드를 포함해주세요:
            {
              "title": "생성된 제목",
              "content": "생성된 블로그 내용",
            ...
            중요: 응답은 반드시 {로 시작하고 }로 끝나는 완전한 JSON이어야 합니다.
            """, userInput, storeInfo);
    }
    ...
    /**
     * 이미지 URL을 포함한 블로그 프롬프트 생성
     */
    private String createBlogPromptWithImages(String userInput, String storeInfo, List<String> imageUrls) {
        StringBuilder imageInfo = new StringBuilder();
        if (imageUrls != null && !imageUrls.isEmpty()) {
            imageInfo.append("\n\n[제공된 이미지]\n");
            for (int i = 0; i < imageUrls.size(); i++) {
                imageInfo.append(String.format("이미지 %d: %s\n", i + 1, imageUrls.get(i)));
            }
            imageInfo.append("※ 이 이미지들을 글 중간에 적절히 배치하여 독자의 시각적 흥미를 높여주세요.");
        }
        ...
        return String.format("""
            ...
            3. 이미지 삽입 규칙:
               - 제공된 이미지 URL을 글 중간에 적절히 배치
               - 이미지 삽입 위치에 [IMAGE:이미지URL] 형식으로 표시
               - 이미지와 관련된 설명을 자연스럽게 포함
               - 각 이미지는 문맥과 연관되도록 배치
               - 예시: "매장 내부는 정말 아늑했습니다. [IMAGE:https://example.com/image1.jpg] 사진에서 보시는 것처럼..."
            ...
            {
              "title": "생성된 제목",
              "content": "생성된 블로그 내용 (이미지 URL 포함)",
              "hashtags": ["해시태그1", "해시태그2", "해시태그3", "해시태그4", "해시태그5"]
            }
            
            중요: 응답은 반드시 {로 시작하고 }로 끝나는 완전한 JSON이어야 합니다.
            """, userInput, storeInfo, imageInfo.toString());
    }
}